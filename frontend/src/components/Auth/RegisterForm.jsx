...
import { userRegisterationPost } from '../../services/Auth';
...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      try {
        const res = await userRegisterationPost(formData);
        navigate("/", {replace: true});
        window.location.reload();
      } catch (error) {
        if (error.response?.status === 409) {
          showError("This email is already registered. Please log in or use another one.");
        } else {
    ...
    <div className="flex items-center justify-center px-4">
      ...
      >
        ...
        <p className="text-gray-400 mb-6 text-center">
          Join us to explore a world of Events and unforgettable experiences.
        </p>
